...
from src.load.db_check_dim_product import db_operations
from src.extract_and_transform.dim_product import get_dim_product
...
def load_dim_product_to_db():
    ...
    try:
        ...
        df_dim_product = get_dim_product()
        ...
        # This case appends the data to the "dim_product" table in the specified schema
        df.to_sql("dim_product", engine, schema=postgres_schema, if_exists="append", index=False)

        length = len(df)
        print(f"Successfully loaded {length} records into dim_product table.")
