...
from src.load.db_check_dim_date import db_operations
from src.extract_and_transform.dim_date import get_dim_date
...
def load_dim_date_to_db():
    ...
    try:
        ...
        # This case appends the data to the "dim_date" table in the specified schema
        df.to_sql("dim_date", engine, schema=postgres_schema, if_exists="append", index=False)

        length = len(df)
        print(f"Successfully loaded {length} records into dim_date table.")
