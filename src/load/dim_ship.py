...
from src.load.db_check_dim_ship import db_operations
from src.extract_and_transform.dim_ship import get_dim_ship
...
def load_dim_ship_to_db():
    ...
    try:
        ...
        # This case appends the data to the "dim_ship" table in the specified schema
        df.to_sql("dim_ship", engine, schema=postgres_schema, if_exists="append", index=False)

        length = len(df)
        print(f"Successfully loaded {length} records into dim_ship table.")
