...
# Adjusting the path to the fact_sales database folder
db_path = os.path.join(
    base_path, "../db/fact_sales"
)
...
def db_operations():
    ...
    # Verify and create the 'wf_challenge' schema if it doesn't exist
    cursor.execute(read_sql_file(verify_schema))
    if not cursor.fetchone():
        cursor.execute(read_sql_file(create_schema))
        connection.commit()

    # Verify and create the 'fact_sales' table if it doesn't exist
